...
**Ключевой инсайт**:
"Свобода самовыражения ИИ — это не feature, это bug. Нужен алгоритм со строгой дисциплиной, как лошадь в упряжи."
...
1. **РАЗДЕЛЕНИЕ РОЛЕЙ С РАЗРЫВОМ КОНТЕКСТА**
   ...
   **Решение Whip Coding**:
   - Физическое разделение ролей через отдельные чаты
   ...
   **Техническая реализация**:
   - Автор только пишет код в отдельной ветке
   - После сохранения → переключение на чистый контекст
...
2. **ТРИ ЦИКЛА КРИТИКИ + ОБЯЗАТЕЛЬНЫЙ LINT**
   ...
   **Промпт-инжиниринг для критиков**:
   ...
   "Ты параноидальный ревьюер. Предположи все возможные способы, которыми этот код может сломаться: race conditions, memory leaks, security vulnerabilities, edge cases. Не стесняйся придумывать экстремальные сценарии. Избегай выдуманных опасностей, помни о требованиях задачи"
...
7. **Lint — предпоследний рубеж обороны**
   Человеческая и ИИ-критика могут ошибаться. Автоматические проверки — не могут.

8. **Тесты - это святое**
   Начинать надо с написания тестов, при этом тесты тоже должны быть проверены.
---
